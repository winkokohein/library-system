...
@section ('title', __('book::labels.backend.book.management') . ' | ' . __('book::labels.backend.book.edit'))
...
@section('content')
{{ html()->modelForm($book, 'PATCH', route('admin.book.update', $book->id))->class('form-horizontal')->open() }}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        ...
                        <small class="text-muted">{{ __('book::labels.backend.book.edit') }}</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
                ...
                <div class="col">
                    ...
                    {{ html()->label(__('book::labels.backend.book.table.name'))->class('col-md-2 form-control-label')->for('name') }}
                        ...
                        <div class="col-md-10">
                            {{ html()->text('book_name')
                                ->class('form-control')
                                ->placeholder(__('book::labels.backend.book.table.book_name'))
                        ...
                        <div class="col-md-10">
                            <select name="author_id" id="author" class="form-control author-name" required="required">
                                ...
                                @foreach ($authors as $key => $author)
                                    <option value="{{ $author->id }}" name="author_id" 
                        ...
                        <div class="col-md-10">
                            <select name="book_category_id" id="book_category" class="form-control book-category-name" required="required">
                                ...
                                @foreach ($book_categories as $key => $book_category)
                                    <option value="{{ $book_category->id }}" name="book_category_id" 
                                        {{ old('book_category_id',$book_category->id ?? '') == $book_category->id ? 'selected' : '' }}>
                                        {{ $book_category->name }}
                                    </option>
                                @endforeach
                            </select>
